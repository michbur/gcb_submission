...
A common model of classical signal peptides assumes that they start with a positively charged n-region, followed by a hydrophobic h-region and a c-region ended with a cleavage site recognised by a signal peptidase. However, studies of many protein sequences representing the wide range of diversified taxonomic organisms indicate a variability of signal peptides. Therefore, we designed a new, more univeral probabilistic model for eukaryotic signal peptides, which includes knowledge about their organisation, amino acid composition and variation.
...
Using the biogram software~\citep{biogramPackage} we extracted n-grams from cleavage sites of signal peptides. The analyzed sequences were already encodeded using amino acid classification providing the best sensitivity of a general model (see Tab.\ref{tab:best}. Selected n-grams, representing less common cleavage site motifs, were included in the HSMM model as the alternative paths at the end of c-region. The achitecture of extended HSMM can be seen in Fig.~\ref{fig:ngramext}.
...
\caption{Results of cross-validation. 1. The encoding providing the best sensitivity (AUC = 0.9683, MCC = 0.8677). 2. The encoding providing the best specificity (AUC = 0.9338, MCC = 0.6474).}
\label{fig:cvres}
\end{figure}
...
The flexibility and efficient information recovery makes our model unique among similar software. The signalHsmm not only shows very good performance but also is adjustable to model properly very specific signal peptides belonging to poorly represented taxonomic groups in databases.

\subsection*{Availability and implementation}
The signalHsmm prediction web-server is avaible under address: \url{http://smorfland.uni.wroc.pl/signalhsmm}.

signalHsmm is implemented as an R package freely available on CRAN: \url{http://cran.r-project.org/web/packages/signal.hsmm}. Stand-alone version offers also tools to build and test own signal peptide models.
