...
Let us first define Hidden Markov Model (HMM). Suppose you have a sequence of observations e.g. amino acids. What is of interest is
...
It is However still not flexible enough. Empirical regions length distributions (see figure~\ref{fig:reglen})
...
The gain is not only a better fit to the data but also, as 
model structure (see figure \ref{fig:model_hsmm}) is straightforward, easier interpretation 
for researcher without mathematical background.

%model structure
\begin{figure}
 \begin{minipage}{.5\linewidth} 
 \caption{Scheme of signalHsmm}
     \resizebox{6.5cm}{!}{%
...
\end{minipage}
 \begin{minipage}{.5\linewidth}
   \caption{Scheme of signalHsmm with cleavage site motives}
     \resizebox{6.5cm}{!}{%
    \begin{tikzpicture}[->,>=stealth',shorten >=2pt,auto,node distance=4.5cm, thick]
...
HSMM can be further extend to use some specific motives that might occur in proximity of cleavage site.
Such an extension requires defining additional hidden states. 
It proves to be extremely useful in recognizing cleavage site.
Scheme can be seen in figure \ref{fig:model_hsmm_kMer}.
